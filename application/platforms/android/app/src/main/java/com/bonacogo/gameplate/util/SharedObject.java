...
public class SharedObject {
    ...
    public static Object getObject(SharedPreferences prefs, String DETAILS_STRING) {
        // prendo l'id che rappresenta l'oggetto nelle SP, lo estraggo e lo restituisco
        // se non esiste ne creo uno nuovo
        Gson gson = new Gson();

        String json = prefs.getString(DETAILS_STRING, null);
        switch (DETAILS_STRING)  {
            case HistoryObject.DETAILS_STRING:
                if(json == null)
                    return new HistoryObject();
                return gson.fromJson(json, HistoryObject.class);
            case FilterFMObject.DETAILS_STRING:
                if(json == null)
                    return new FilterFMObject();
                return gson.fromJson(json, FilterFMObject.class);
            case FilterSObject.DETAILS_STRING:
                if(json == null)
                    return new FilterSObject();
                return gson.fromJson(json, FilterSObject.class);
            case ListGameObject.DETAILS_STRING:
                if(json == null)
                    return new ListGameObject();
                return gson.fromJson(json, ListGameObject.class);
            case MyLastLocationObject.DETAILS_STRING:
                if(json == null) {
                    return null;
                }
                return gson.fromJson(json, MyLastLocationObject.class);
            default:
                return null;
        }
    }

    public static HashMap<String, LatLng> getHashMapCountry() {
    ...
    // salva l'oggetto nelle SP
    public static void saveObject(Object obj, SharedPreferences prefs, String DETAILS_STRING) {
        SharedPreferences.Editor prefsEditor = prefs.edit();
        Gson gson = new Gson();
        String json = gson.toJson(obj);
        prefsEditor.putString(DETAILS_STRING, json).apply();
    }
}
